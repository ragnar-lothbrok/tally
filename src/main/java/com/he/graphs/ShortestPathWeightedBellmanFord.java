...
public class ShortestPathWeightedBellmanFord {
	...
	public static void main(String[] args) throws IOException {
		...
		// int result = weightedShortestPath(adajacency, path, distance, source,
		// vertexList);
		int result = bellManFord(adajacency, path, distance, source, vertexList);
		if (result == -1) {
	...
	public static int bellManFord(int adajacency[][], int path[], long distance[], int sourceIndex, List<Integer> vertexList) {
		for (int i = 0; i < adajacency.length; i++) {
		...
		for (int i = 0; i < adajacency.length - 1; i++) {
			for (int source = 0; source < adajacency.length; source++) {
				for (int destination = 0; destination < adajacency.length; destination++) {
					if (adajacency[source][destination] != 0) {
						if (distance[destination] > distance[source] + adajacency[source][destination]) {
							distance[destination] = distance[source] + adajacency[source][destination];
	...
	public static int weightedShortestPath(int adajacency[][], int path[], long distance[], int source, List<Integer> vertexList) {
		int k = 0, current;

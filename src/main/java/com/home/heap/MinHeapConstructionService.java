...
public class MinHeapConstructionService<T> {
    ...
    public void addoToHeap(HeapNode<T> heapNode) {
        if (heapNodesList.size() == 0) {
            ...
            if (currentPos != -1) {
                ...
                while (parentPos >= 0 && heapNodesList.get(parentPos).getWeight() > heapNodesList.get(currentPos).getWeight()) {
    ...
    public void hepify(Integer position) {
        if (position >= 0 && position < heapNodesList.size()) {
            while (true) {
                ...
                int minValueIndex = (heapNodesList.get(left).getWeight() < heapNodesList.get(right).getWeight() ? left
                        : right);
                if (heapNodesList.get(minValueIndex).getWeight() < heapNodesList.get(position).getWeight()) {
                    swap(position, minValueIndex);
                    position = minValueIndex;
                } else {
                    break;
                }
            }
